...
class WardWiseBeatController extends GetxController{
  GoogleMapController? mapController;
  final CreatedBeatController createdBeatController =
  Get.find<CreatedBeatController>();
  RxList<BeatData> beatListToShow =<BeatData>[].obs;
  final Map<PolylineId, BeatData> polylineToBeatMap = {};
  Rxn<PolylineId> highlightedPolylineId = Rxn<PolylineId>();
  final Rxn<BeatData> selectedBeatData = Rxn<BeatData>();
  ...
  void onInit() {
    super.onInit();
    beatListToShow.assignAll(createdBeatController.beatListAll);
        ...
        if (points.isNotEmpty) {
          ...
          markers.add(
            Marker(
              ...
              position: midpoint,
              infoWindow: InfoWindow(title: beat.subBeetName ?? '', snippet: "${beat.distance.toString()} Mtrs"),
              icon: BitmapDescriptor.defaultMarkerWithHue(BitmapDescriptor.hueRose), // Optional: custom color
            ),
          );
        }
      }
